...
The only issue I had with A is finding an efficient way to find all powersets. Fortunately, using `chain` and `combinations` from `itertools` gave all subsets of rules in increasing cardinality, so all I had to do was break when the symmetric differences of all the rules in the subset was equal to the locations where there were 
...
Suppose we had $n$ machines and $k$ buttons. Let $A\in\mathbb Z^{n\times k}$. We define $a_{ij}$ to be 1 if button $j$ toggles machine $i$ and 0 otherwise. Then let $b\in \mathbb Z^n$, where $b_i$ is the joltage of machine $i$. Then our solution is the $x\in \mathbb Z ^k$ with $Ax=b$ and $x_i \geq 0$ that minimizes $||x||_1 = \sum x_i$. 
