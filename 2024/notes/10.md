...
Fun! Not bad at all, just for each 0: find each neighboring 1, then find each neighboring 2, etc. 

## Part B

Just had to change them from sets to dicts with the keys as coordinates and the vals as the number of ways you could reach that point from the original 0 that we start at. Wasn't bad at all
